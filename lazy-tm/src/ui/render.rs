...
fn build_instructions(mode: &Mode) -> Line<'static> {
    match mode {
...
fn render_editing_inputs(frame: &mut Frame, state: &UiState, input_area: Rect) {
...
pub fn render(frame: &mut Frame, state: &mut UiState, app: &mut App) {
    ...
    let instructions = build_instructions(&state.mode);
    ...
    if state.mode == Mode::Editing {
        ...
        render_editing_inputs(frame, state, input_area);
    } else {
